...
unsigned int Pyuv422tobgr444DownSample(const unsigned char *input_ptr, unsigned char * output_ptr, unsigned int input_image_width, unsigned int input_image_height)
{
	...
	unsigned int image_width_2;
	unsigned int Y_moy, U_moy,V_moy;
	
	const unsigned char *buff = input_ptr;
	const unsigned char *buff_under = input_ptr;
	unsigned char *output_pt = output_ptr;
	j_max = (input_image_height)/2;
	i_max = input_image_width /2;
	image_width_2 = input_image_width*2;
	for (j=0; j<j_max;++j)
	{
		buff_under= buff + image_width_2;
		for (i=0; i< i_max; i++)
		{
			Y_moy=0;
			Y_moy+=buff[0];
			...
			*output_pt++ = B_FROMYU(Y_moy,U_moy); //v
			*output_pt++ = G_FROMYUV(Y_moy,U_moy,V_moy); //b
			*output_pt++ = R_FROMYV(Y_moy,V_moy);
...
unsigned int
Pyuv422tobgr24(unsigned char * input_ptr, unsigned char * output_ptr, unsigned int image_width, unsigned int image_height)
{
	unsigned int i, size;
	unsigned char Y, Y1, U, V;
	unsigned char *buff = input_ptr;

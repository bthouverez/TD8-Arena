	     ...
	     int format, int grabmethod, char *avifilename)
{
   int ret = -1;
	...
	vd->isstreaming = 0; //EL+, pour close_v4l2
    vd->tmpbuffer = NULL; //EL+, pour close_v4l2
    vd->framebuffer = NULL; //EL+, pour close_v4l2
...
/*------------------------------------*/
/*  version modifiee par brice et EL  */
/*------------------------------------*/
  ...
  //subroutine to read menu items of controls with type 3
	...
	for (queryctrl.id = V4L2_CID_BASE;queryctrl.id < V4L2_CID_LASTP1;queryctrl.id++)
	{
		...
		if (0 == ioctl (vd, VIDIOC_QUERYCTRL, &queryctrl))
		{
			...
			printf ("   name:%-32s type:%d min:%-5d max:%-5d step:%-5d def:%-5d now:%d\n",
				queryctrl.name, queryctrl.type, queryctrl.minimum,
		...
		{
			printf( " non gere par le peripherique.\n");
			if (errno == EINVAL)
				continue;
	...
	printf ("Driver specific controls (V4L2_CID_PRIVATE_BASE) :\n");
	...
	printf ("Driver specific controls (V4L2_CID_CAMERA_CLASS_BASE) :\n");
	for( queryctrl.id = V4L2_CID_CAMERA_CLASS_BASE+1;; queryctrl.id++)
	{
		printf( " code=%#0x", queryctrl.id);
...
/* version d'origine de luvcview */
int enum_controls_bak(int vd) //struct vdIn *vd)
{
  struct v4l2_queryctrl queryctrl;
  struct v4l2_querymenu querymenu;
  struct v4l2_control   control_s;
...
int load_controls(int vd) //struct vdIn *vd)
{
  ...
  if ( configfile == NULL) {
    ...
    char buffer[512];
	char *dummy;
    dummy = fgets(buffer, sizeof(buffer), configfile);
...
static int init_v4l2(struct vdIn *vd)
{
	int i;
	int ret = 0;
	struct v4l2_streamparm* setfps = NULL; //EL deplacé en debut de bloc
	...
	/* set framerate */
	//EL- struct v4l2_streamparm* setfps = NULL;
...
int v4l2GetControl(struct vdIn *vd, int control)
{
    ...
    struct v4l2_control control_s;
    int err;
    // isv4l2Control fails with pro9000 / ubuntu 10.10
...
/*
    ...
    // isv4l2Control fails width pseye
    //if (isv4l2Control(vd, control, &queryctrl) < 0)
	//return -1;
